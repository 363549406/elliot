...
* Branch References：分支指向某一次commit (SHA-1) 。一个底层的命令可以查看分支指向的commit
   ...
   ```
* RefLog Shortnames：**存储在本地**，新clone到本地的仓库，reflog日志为空。用来记录最近几个月HEAD和branch ref所指向的历史。
   ```
   git reflog   # 查看所有日志
   git show HEAD@{5}  # HEAD最近第5个commit
   git show master@{yesterday} # master昨天所指向的commit
   ```
* Ancestry References: 祖先ref。使用`^`和`~`，却别是`^2`指merge中的第二个分支，`~2`指父分支的父分支。
   ```
   ...
   * 9b29157 add open3_detach to gemspec file list
   ...
   $ git show HEAD~2 # the parent of parent of HEAD
   $ git show HEAD~~ # the parent of parent of HEAD
   $ git show HEAD^^ # the parent of parent of HEAD
   ```
* Commit Ranges：如何查看一定区间的提交。
   * 双点：在后一个分支中但不在前一个分支中的提交
      ```
      $ git log master..develop  # 查看没合并到master的commit
      $ git log mine/develop..develop  # 查看会提交到远端的commit
      ```
   * Multiple Points：seeing what commits are in any of serveral branches that aren't in the another branch. 在不想要的分支前加`^`或`--not`。
      ```
      ...
      ```
   * Triple Dot: specifies all the commits that are reachable by __either__ of two references but not by both of them.
      ```
      $ git log master...develop
      $ git log --left-right master...develop # shows you which side of the range each commit is in.
      ```
